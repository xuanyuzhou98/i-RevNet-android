...
public class Main {
    ...
    private static final String basePath = System.getProperty(
            "user.home") + "/mnist";
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            log.info("Flop count", "batch size " + batchSize);
            log.info("Flop count", "forward count " + totalFlopsForward);
            log.info("Flop count", "bacward count " + totalFlopsBackward);
            ...
            log.info("evaluate stats", eval.stats());
            log.info("finished", "****************Example finished********************");
        } catch (Exception e) {
            System.out.println(e);
        }
    }

    private static int[] getConvLayerOutShape(int inputH, int inputW, int 
            filterSize, int stride) {
        int outputH = (inputH - filterSize) / stride + 1;
        int outputW = (inputW - filterSize) / stride + 1;
        return new int[] {outputH, outputW};
    }

    private static int getFlopCountConv(int channels, int filter_size, int 
            num_filters,
                                 int outShapeH, int outShapeW) {
        return (2 * channels * filter_size * filter_size - 1) * num_filters * outShapeH * outShapeW;
    }

    private static int getFlopCountFC(int inputSize, int outputSize) {
        return  (2 * inputSize - 1) * outputSize;
    }

    private static int getFlopCountConvBackward(int channels, int filter_size,
                                              int num_filters,
                                         int outShapeH, int outShapeW) {
    ...
    private static int getFlopCountFCBackward(int inputSize, int outputSize) {
        int db = outputSize;
